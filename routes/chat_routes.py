...
def _stream_set(session_id: str, **fields) -> None:
    """更新 `session_id` 的活动流条目中的字段，如果条目已被弹出则为无操作。
    使用 .get() 避免了在 `if x in d` 和 `d[x]["k"] = v` 之间的 KeyError 竞态条件——
    即当兄弟协程的 finally 在中间弹出了键时（这种情况可能在协程取消
    到达内部清理时发生，比最外层清理更早执行）。"""
...
def _is_image_generation_session(sess, owner: str | None = None) -> bool:
    ...
    模型名称前缀是显式图像模型。端点类型仅在当前会话
    端点实际匹配该图像端点且已填充的端点模型缓存包含所选模型时使用。
    这防止同一主机上的图像端点将普通文本模型错误路由到图像生成路径。
    """
...
def _recover_empty_session_model(sess, session_id: str, owner: str | None = None) -> bool:
    ...
    覆盖端点设置与首次聊天发送之间的窗口：选择器在下拉列表中显示了模型，
    但会话记录从未写入（Issue #587 — UI 使用缓存的端点列表，而非 s.model）。
    对于 ChatGPT Subscription，也会修复过时的 OpenAI API 模型名称，
    如 ``gpt-5``（Codex 支持的 ChatGPT 账户路由不接受这些名称）。
    """
    ...
    db = SessionLocal()
    try:
        # 优先选择 base URL 与会话匹配的端点 — 已知用户已将这个会话
        # 指向该端点，因此其第一个缓存的模型是最合理的默认值。
        ep = None
...
def setup_chat_routes(
    ...
    async def chat_stream(request: Request) -> StreamingResponse:
        ...
        try:
            ...
            if _clear_orphaned_session_endpoint(sess, owner=owner):
                raise HTTPException(400, "Selected model endpoint was removed. Pick another model in Settings.")
            # Issue #587：选择器显示了端点缓存中的模型，但
            # s.model 从未写入到数据库行中（端点设置后的首次发送竞态，
            # 或之前的端点删除/重建）。从匹配端点的第一个缓存模型中
        ...
        async def stream_with_save() -> AsyncGenerator[str, None]:
            ...
            # 通知在预处理期间自动创建的文档（例如可填写的
            # PDF → 可编辑 markdown），以便编辑器窗格在模型开始流式传输前切换到它们。
