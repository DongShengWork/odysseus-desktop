...
export function init(documentModule) {
  ...
  initEmailLibrary({
    ...
    onEmailClick: async (opts) => {
      // 回复 / AI 回复 / 撰写在文档编辑器中打开草稿。
      //  - 桌面端：将邮件停靠到左侧，使其在回复草稿（在右侧打开）旁边
      //    保持可见 — 边读边回复。
      //  - 移动端：没有空间进行分屏，因此最小化邮件模态框；
      //    草稿显示在前面，收件箱以最小化芯片形式一键可达。
      ...
      try {
        if (Modals.isRegistered('email-lib-modal')) {
          ...
          // 移动端：不要在此处预挂载窗格。加载路径（open/inject）
          // 在文档准备好时恰好挂载一次；doc-view z-index 规则将其
          // 滑动到邮件之上（邮件停留在后面）。在此处预挂载会导致
          // 双重挂载 — 早期窗格被撰写会话切换拆除，然后重新挂载，
          // 看起来像是在平滑滑动之前文档闪烁了一下。
        }
      } catch (_) {}
...
// 当文档编辑器窗格打开时（body.doc-view 打开），确保
// 邮件模态框在左侧 — 即使它之前停靠在右侧或浮动 —
// 这样邮件和文档始终并排显示。实际的宽度计算在 modalSnap.js 中
//（`_anchorLeftDock` 在文档渲染到右侧时会缩小邮件窗口）。
...
async function _openEmail(em, itemEl, preloadedData = null, mode = 'reply') {
  ...
  if (wantsAiReply) {
    // 回退到回复全部（而非普通回复），这样生成的 AI 草稿
    // 会覆盖原始线程上的所有人。对于单个收件人的邮件，
    // 这与普通回复效果相同，因为没有其他人需要抄送。
    mode = 'reply-all';
  }
  ...
  try {
    ...
    if (_docModule) {
      // 只有当用户真的只想"查看"邮件时才重用现有文档标签页。
      // 对于回复/回复全部/转发/ai-reply，始终创建新草稿 —
      // 否则之前已清空的文档（已发送的回复、返回空白的 AI 回复等）
      // 会再次出现，而不是正确的预填充回复。
      ...
      if (existingDocId) {
        ...
        if (!activeSid) {
          // 没有正在进行的聊天 — 保留创建作用域邮件线程聊天的旧行为，
          // 然后重新读取当前的会话 ID。下面的 POST 需要 session_id
          //（后端没有它返回 400），刚创建的聊天才是回复草稿的所有者。
        ...
        if (doc.id) {
          ...
          await new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)));
          // 直接使用 POST 返回的 doc 字典 — 避免在 GET 触发时
          // 新行尚未对读连接可见（或缓存干扰）导致的 404 竞态。
          // loadDocument 的 GET 路径仍可用作回退。
...
function _showRemindSubmenu(em, parentDropdown) {
  ...
  customItem.addEventListener('click', async (e) => {
    ...
    // 点击外部关闭。替换旧的基于 blur 的自动删除 —
    // 每当原生日期时间选择器抢占焦点时，blur 就会触发，因此在用户
    // 点击任何日期之前输入框就消失了。现在只有在用户点击不是输入框
    // 本身的东西时才关闭（原生选择器弹出窗口是浏览器拥有的
    // 文档外覆盖层，因此其点击不会在这里触发 — 不会误关闭）。
      ...
      try {
        ...
        if (_sres.ok) {
          ...
          if (_sdata && _sdata.id) {
            ...
            // 重新选择（空的）会话会重新渲染聊天并在草稿打开前闪烁
            // 欢迎画面一帧 — "欢迎画面闪烁得厉害然后邮件才打开"的 bug。
            // 文档只需要 session_id；无论如何草稿都在文档面板中打开。
          }
        }
