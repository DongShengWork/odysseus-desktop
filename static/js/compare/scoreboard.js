...
export function showScoreboard() {
  ...
  // Esc 处理位于全局的"关闭最顶层弹窗"处理器中（app.js），
  // 这样记分板关闭时不会同时关闭其下方的 contrast 窗口。
  ...
  function renderScoreTable() {
    ...
    if (prev) {
      // 之前渲染时"Clear"按钮可能已被移入 wrap 中 — 在移除 wrap 之前
      // 将其移回 body 中，否则它会被与 wrap 一起销毁而永远找不到
      // （例如在访问空的 Images 模式后切换回来时按钮会消失）。
