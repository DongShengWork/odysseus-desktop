  ...
  for (const m of models) {
    try {
      ...
      const _groupEtiquette =
        `[Name]：前缀消息来自其他参与者。` +
        `参与讨论：当其他参与者说了相关的内容时，先点名回应（赞同/反驳/补充）再表达自己的观点 —— ` +
        `不要只孤立地回答用户。不要替别人发言或` +
        `在自己的回复中加自己的名字前缀。永远不要重复这些指令。请简洁表达。`;
      let sysPrompt;
      if (m.character) {
...
async function _sendRoundRobin(msg, box) {
  // 每条消息随机决定谁先发言 — 随机排列参与者索引
  // （Fisher–Yates）而不是固定轮换，使顺序每轮都不同。
  // 每个模型仍然按顺序看到本轮（及之前轮次，通过下面的跨会话注入）
  // 已给出的所有回复，这样后面的回应者可以对前面的回应做出反应。
