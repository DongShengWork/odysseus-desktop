...
//
// 所有函数都是 HTML 字符串的纯函数（有一个例外是操作 DOM 的：
// `_harvestAttribution` 从容器中剥离节点）。无模块状态、
// 无 fetch、无全局变量。图标（`_SIG_ICON`、`_QUOTE_ICON`）也放在这里，
// 因为 `_foldSummary` 是唯一调用方，其他模块通过该辅助函数传入。
...
// 看起来像签名/公司免责声明而非引用邮件。
// 启发式算法：对已知的"这是免责声明"信号和"这是真实邮件"信号进行
// 评分。3 个以上免责声明命中且少于等于 1 个对话命中 → 签名。
...
// 顶级签名折叠 — 按优先级顺序运行多种检测策略。
// 当没有任何策略匹配时，返回原始 html 不变。
