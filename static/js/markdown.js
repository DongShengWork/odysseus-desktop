...
/**
 * 清理 mdToHtml 从源文本中有意保留的原始 HTML 片段——
 * <details> 块（可折叠的 agent 输出）和 <a> 标签（由 markdown 链接处理生成）。
 * 这些片段后续会被原样恢复到 innerHTML 中，因此如果不清洗，模型——
 * 或任何经过此处的内容——可以夹带 `<img onerror=...>`、
...
const _ALLOWED_HTML_BAD_TAGS = new Set([
  ...
  // 外来内容根元素。SVG/MathML 有自己的解析规则，是经典的
  // 变异型 XSS 载体——例如 SVG 命名空间的 <script>，
  // 其 `tagName` 是小写的 'script'，会绕过假设 HTML 大写命名的检查。
  // 在我们保留的 <details>/<a> 片段中不需要它们，因此移除整个子树。
  'SVG', 'MATH',
]);
...
export function extractThinkingBlocks(text) {
  ...
  // 如果存在未关闭的标签，分两种情况处理：
  // (a) 开头有一个独立的开始标签，之前没有真实的回复内容——典型于
  //     量化模型（MiniMax-AWQ），它们每次回复的开头都输出一个字面的 `<think>` 标记
  //     而从不关闭它。只需移除开始标签，保留内容作为回复，
  //     否则气泡在重载时看起来是空白的（内容被视为折叠的思考）。
  // (b) 生成中途截断——在开始标签之前已有真实的回复文字。
  //     从标签位置截断（这是被截断的思考内容）。
...
export function mdToHtml(src, opts) {
  ...
  // 在任何 markdown/HTML 保留处理之前，先提取围栏代码块。
  // 否则，允许的 HTML 清理器生成的占位符（例如
  // ___ALLOWED_HTML_0___）可能泄漏到引用的 HTML/JS 示例中，
  // 因为占位符被原样捕获并永远不会在最终的 <pre><code> 块中还原。
  s = s.replace(/```(\w+)?\n([\s\S]*?)```/g, (_, lang, code) => {
    const cleaned = code
      .replace(/\r\n/g, '\n')
      .replace(/[ \t]+$/gm, '')
  ...
  // 修复 agent 破坏实体锚点约定 (`[Name](#kind-<id>)`) 的常见方式。
  // 模型在单链接情况下通常正确，但在表格中列出多个时会使用其他格式。
  // 这些正则表达式将错误的格式升级为正确的 markdown 链接，
  // 以便后续标准的 `[text](url)` 处理器能正确拾取。
