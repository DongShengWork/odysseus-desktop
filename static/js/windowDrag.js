...
// CSS 变量查找图标栏+侧边栏宽度 — 用于在全屏拖出时确定
// "左边缘"的实际位置（光标需要越过图标栏才算"靠近左侧"）。
...
export function makeWindowDraggable(modal, options = {}) {
  ...
  // 左停靠默认也启用。modalSnap 折叠宽侧边栏
  // 并将面板锚定在图标栏旁边，这样不会与导航冲突。
  // 调用者仍可通过 enableLeftDock:false 禁止特殊模态框的左停靠。
  ...
  const _onMove = (cx, cy) => {
    ...
    // 而不是把模态框丢在半空中。
    if (_isFullscreen()) {
      // 角落守卫：当光标仍在顶部全屏条带内时，忽略侧边边缘，
      // 这样拖过顶部角落时保持全屏而不是翻转为角落停靠。
      ...
      // 将全屏窗口拖到侧边边缘 → 保持全屏并且仅
      // 启用侧边停靠提示；在那里释放则停靠它（在 _onEnd 中处理，
      // 那里会移除全屏 class）。此前这会先退出全屏，
      // 然后重新居中窗口 — 所以看起来像"在中间而不是停靠"。
      // 只有向下拖拽才会取消吸附到窗口化（居中）的模态框。
