...
// 当笔记的到期时间已过且用户尚未归档或完全完成它时，该笔记有活跃提醒。
// 用于排序（提升到未固定部分的最前面）和 entry-glow 刷新。
...
// 将选择的日期时间向前对齐到匹配标准化重复模式的下一个时间点
// （保留当日时间，严格在未来）。当用户选择的日期在未来时锚定到该日期
// （这样在遥远的未来日期上选择重复不会把它拖回今天）；否则
// 锚定到"现在"。对 daily/yearly/none 返回 null。
...
function _fireReminder(note) {
  ...
  // 请求服务器根据用户设置进行分发。服务器可能返回 LLM 生成的摘要
  // 和/或发送电子邮件。我们仍然显示本地浏览器通知，
  // 使用户即使服务器路径被禁用或缓慢也能获得即时反馈。
  ...
  // 发射后不管式服务器分发。如果合成结果回来得足够快，
  // 用它作为通知正文；否则本地通知已经以原始正文显示了。
  let shown = false;
...
export function openPanel() {
  ...
  // Escape：先退出选择模式（如果处于激活状态），否则关闭面板。
  // 当用户正在编辑表单字段时跳过 — 这些字段有自己的
  // ESC 取消处理器，我们不希望在编辑中销毁整个面板。
  // 幂等性：移除之前 openPanel 中的任何旧处理器，
  // 避免重新打开时堆叠多个处理器。
  if (_notesKeydownHandler) {
    document.removeEventListener('keydown', _notesKeydownHandler);
    _notesKeydownHandler = null;
...
// ── 草稿自动保存 ──────────────────────────────────────────────────
// 当笔记在编辑器中打开时，其表单会在每次更改时快照到
// localStorage（防抖）。如果连接中断、标签页关闭或页面在点击保存前
// 重新加载，重新打开该笔记会恢复未保存的文本。草稿在显式保存或
// 取消时清除。由于不涉及网络，离线时也能存活。
...
function _buildForm(note = null) {
  ...
  // 存储原始表单值，使往返切换（Note→Todo→Note）时恢复
  // 用户手动格式化的文本，而不是生成项目的拼接。反之亦然：
  // 如果你从 todo 开始，切换到 note，再切换回来，项目保持不变。
  ...
  if (photoBtn && photoInput) {
    ...
    photoInput.addEventListener('change', async () => {
      ...
      try {
        ...
        // 永远只保留最新附件照片 — 在插入新照片之前
        // 删除任何现有的包裹容器。选择第二张照片替换第一张而不是堆叠。
  ...
  _saveBtnEl0.addEventListener('click', async () => {
    ...
    if (!payload.pinned) {
      // 编辑和新建笔记都锚定在未固定区域的其余部分之上。
      // 没有这个，新创建的笔记会位于底部，因为手动重新排序的
...
// 拖起-占位符拖拽实现。被拖拽的卡片从网格中分离
//（position:fixed，锚定到手指），而一个相同尺寸的
// 占位符占据其位置。只有占位符在手指越过中点时在兄弟
// 元素间移动 — 卡片在拖动期间从不重新定位父级，
// 这消除了之前每帧交换实现中的振荡/跳动问题。
...
function _onDocTouchEnd() {
  ...
  setTimeout(() => {
    ...
    // 恢复卡片拖拽前的内联样式。移动端瀑布流将
    // grid-row-end 存储在内联样式中，自定义背景也使用内联样式；
    // 清除 cssText 会导致放置的卡片在网格视图中塌缩到
    // 邻近笔记中。
...
async function openNote(noteId) {
  ...
  // 可能还未出现在 DOM 中。也直接轮询 _notes 模块数组 —
  // 如果笔记已加载但活跃过滤器（如归档视图）正在隐藏它，
  // 我们仍然可以显示确认提示。
  if (!noteId) return;
  let tries = 0;
  const findAndFlash = () => {
