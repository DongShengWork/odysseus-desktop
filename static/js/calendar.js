...
async function _createEventReminder(ev, dueDate) {
  // 将提醒存储为绝对 UTC 时刻（带 Z 后缀），以便通知轮询在正确的挂钟时间触发，不受以下因素影响：
  //   - 事件的源时区（CalDAV/导入可能带有 TZID），
  //   - 用户当前本地时区与创建提醒时不同，
  //   - 下游任何朴素的 ISO 误解。
  // notes.js 和日历轮询都已使用 `new Date(due_date)`，
  // 它能正确处理带 Z 后缀的 ISO 并在显示时转换回本地时间。
  ...
  const payload = {
    ...
    due_date: iso,
    source: 'calendar',
    // 持久化事件的绝对开始时间，以便通知正文在触发时实时计算
    // （"5 分钟后开始"），而非使用调度时写入的过期字符串。
...
function _headerHTML() {
  ...
  <div class="cal-quickadd-row" id="cal-quickadd-row">
    <input
      ...
      autocomplete="off"
    />
    <span class="cal-quickadd-hint" id="cal-quickadd-hint" aria-hidden="true"><span class="qa-hint-accent">${t('calendar.quick_add')}</span> — return home to Ithaca 1pm tmrw <svg class="qa-hint-enter" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="9 10 4 15 9 20"/><path d="M20 4v7a4 4 0 0 1-4 4H4"/></svg></span>
