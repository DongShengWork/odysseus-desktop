...
def _params_from_config(cfg):
    ...
    估算是有意粗略的 — 对于标准 decoder-only 架构在真实数量的
    ~5-10% 以内 — 这对下游的 ``min_vram_gb`` 启发式算法已经够了
    （它已经通过 ``parameter_count`` 分桶到一位小数 "B"）。
    """
        ...
        for target in config_targets:
            ...
            if ct:
                ...
                if ca and active is None:
                    active = ca
                break
    # 最后的手段：读取 safetensors 元素计数。对于预量化的仓库
    #（AWQ/GPTQ/MLX-Int4 等），权重被打包：每个 I32 元素 8×4-bit 权重，
    # 每个 I32 4×8-bit 权重。因此裸 safetensors 总数按相同因子
    # 低估了真实参数数量，进而导致下游错误的 `min_vram_gb`。
    # 按 dtype 求和并解包打包的 I32 张量，使目录存储真实的参数数量。
    if total is None:
        try:
