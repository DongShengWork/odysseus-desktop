...
# 跨平台 OS 标志，在此暴露以便调用者 `from src.config import IS_WINDOWS`。
# 本地定义（简单的 `os.name == "nt"`）而非从 core.platform_compat 导入，
# 以避免此依赖轻量的配置模块拖入整个 core/__init__ + llm_core 导入链。
# 平台*辅助函数*（safe_chmod、pid_alive、find_bash 等）仅存在于
# core.platform_compat — 那里是它们的唯一事实来源。保持平台分支为小型的
